...
abstract class RazorpayBase extends AbstractPaymentProvider<
  RazorpayProviderConfig & Options
> {
  ...
  protected constructor(
    ...
    options: RazorpayProviderConfig & Options
  ) {
    ...
    if (!this.options_.key_id && this.options_.providers?.length) {
      ...
      if (provider) {
        this.options_ = provider?.options as unknown as RazorpayProviderConfig &
          Options
      }
    }
  ...
  async _pollExistingRazorpayCustomer(
    ...
    try {
      ...
      this.logger.error(
        "unable to fetch customers in the razorpay payment processor: " +
    ...
    for (const customerObj of customerList) {
      if (
        customerObj.contact === customer.phone ||
    ...
    if (!customerList.length || !razorpayCustomer) {
      ...
      throw new Error(
        `could not find customer in razorpay: Customer: ${JSON.stringify(
          ...
          null,
          4
        )} : Customer List: ${JSON.stringify(
          customerList,
          null,
          4
        )} : Razorpay Customer: ${JSON.stringify(
          razorpayCustomer,
          null,
          4
  ...
  async getRazorpayCustomer(
    ...
    if (!this._razorpayCustomer) {
      this._razorpayCustomer = await this._pollExistingRazorpayCustomer(
        customer
      )
    }
  ...
  async updateExistingRazorpayCustomer(
    ...
    const razorpayCustomer: Customers.RazorpayCustomer | undefined =
      await this.getRazorpayCustomer(intentRequest, customer)
    ...
    if (razorpayCustomer) {
      ...
      try {
        this._razorpayCustomer = await this.razorpay_.customers.edit(
          razorpayCustomer.id,
          {
  ...
  async createOrUpdateRazorpayCustomer(
    ...
    if (razorpayCustomer) {
      ...
      this.logger.info(
        "customer doesn't exist in razopay. so, creating a new customer"
      )
  ...
  _validateSignature(
    ...
    const expectedSignature = crypto
      .createHmac(
        "sha256",
        this.options_.key_secret ?? (this.options_.key_secret as string)
      )
  ...
  async getRazorpayPaymentStatus(
    ...
    const authorisedRazorpayPayments: Payments.RazorpayPayment[] =
      attempedRazorpayPayments.items.filter(
        (i) => i.status == PaymentSessionStatus.AUTHORIZED
      )
    const totalAuthorised: number = authorisedRazorpayPayments.reduce(
      (p, c) => {
        p += parseInt(`${c.amount}`)
        return p
      },
  ...
  async initiatePayment(
    ...
    this.logger.info(
      `Initiate payment input: ${JSON.stringify(input, null, 4)}`
    )
    ...
    const storeCart: HttpTypes.StoreCart =
      data.extra as unknown as HttpTypes.StoreCart
    const intentRequestData: Partial<PaymentIntentOptions> =
      this.getPaymentIntentOptions()
    ...
    const phoneNumber =
      customerDetails.phone ?? storeCart.billing_address?.phone
    ...
    try {
      ...
      throw this.buildError(
        "An error occurred in InitiatePayment while creating/updating customer data:" +
          e.message,
        e
      )
    }
  ...
  async capturePayment(
    ...
    const razorpayOrder: Orders.RazorpayOrder =
    ...
    const possibleRazorpayPaymentCaptures: Payments.RazorpayPayment[] =
      razorpayOrderPayments.items?.filter((item) => item.status == "authorized")
    const result = possibleRazorpayPaymentCaptures?.map(
      async (possibleRazorpayPaymentCapture) => {
        ...
        const paymentCapture: Payments.RazorpayPayment =
          await this.razorpay_.payments.capture(id, toPay, currency as string)
  ...
  async refundPayment(input: RefundPaymentInput): Promise<RefundPaymentOutput> {
    ...
    const filteredRazorpayPaymentToRefund:
      | Payments.RazorpayPayment
      | undefined = razorpayOrderPayments.items?.find((p) => {
      return (
    ...
    if (filteredRazorpayPaymentToRefund?.id) {
      ...
      try {
        ...
        const refundsIssued: Refunds.RazorpayRefund[] = input.data
          .refundSessions as Refunds.RazorpayRefund[]
  ...
  async retrievePayment(
    ...
    try {
      ...
      const orderId = (data as unknown as Payments.RazorpayPayment)
        .order_id as string
      try {
  ...
  async updatePayment(input: UpdatePaymentInput): Promise<UpdatePaymentOutput> {
    ...
    const billing_address: Partial<AddressDTO> | null =
  ...
  async updatePaymentData(
    ...
    try {
      const razorpayPaymentId: string = (data.data as Record<string, any>)
        .id as string
      const razorpayPayment: Payments.RazorpayPayment =
        await this.razorpay_.payments.fetch(razorpayPaymentId)
      if (data.notes || (data.data as any)?.notes) {
        const notes = data.notes || (data.data as any)?.notes
        const editedRazorpayOrder: Orders.RazorpayOrder =
          await this.razorpay_.orders.edit(sessionId, {
        ...
        razorpayPaymentOutput =
          razorpayPayment as unknown as UpdatePaymentOutput
      }
    } catch (e) {
      this.logger.error(
        "unable to update payment data in razorpay: " +
          JSON.stringify(e, null, 4)
      )
  ...
  async getWebhookActionAndData(
    ...
    try {
      ...
      this.logger.error(
        `Razorpay webhook validation failed : ${JSON.stringify(error, null, 4)}`
      )
